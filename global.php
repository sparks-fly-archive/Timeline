...
/* + PL:patches + */ if($mybb->usergroup['cancp'] == "1") {
/* + PL:patches + */ $uid = $mybb->user['uid'];
/* + PL:patches + */ $query = $db->query("SELECT * FROM mybb_team WHERE uid = '$uid'");
/* + PL:patches + */ $aufgabe = $db->fetch_array($query);
/* + PL:patches + */ 
/* + PL:patches + */ 		$aufgaben = array(
/* + PL:patches + */ 			"technique" => "Technik & Design",
/* + PL:patches + */ 			"avatar2go" => "Avatar 2 Go - Datenbank",
/* + PL:patches + */ 			"faceclaims" => "Avatarpersonendatenbank",
/* + PL:patches + */ 			"graph_awards" => "Awards (Graphics)",
/* + PL:patches + */ 			"code_awards" => "Awards (Coding)",
/* + PL:patches + */ 			"writing_awards" => "Awards (Writing)",
/* + PL:patches + */ 			"code_challenge" => "Challenges (Coding)",
/* + PL:patches + */ 			"graph_challenge" => "Challenges (Graphics)",
/* + PL:patches + */ 			"writing_challenges" => "Challenges (Writing)",
/* + PL:patches + */ 			"coding_school" => "Coding School",
/* + PL:patches + */ 			"code_support" => "Coding-Support verwalten",
/* + PL:patches + */ 			"spam" => "Community & Spambereich",
/* + PL:patches + */ 			"events" => "Foren-Events",
/* + PL:patches + */ 			"requests" => "Gesuchsbereich",
/* + PL:patches + */ 			"graph_requests" => "Grafik-Anfragen verwalten",
/* + PL:patches + */ 			"mentoring" => "Grafik-Mentoring",
/* + PL:patches + */ 			"graph_support" => "Grafik-Support verwalten",
/* + PL:patches + */ 			"graph_tutorials" => "Grafik-Tutorials verwalten",
/* + PL:patches + */ 			"code_hfh" => "Head for Heights (Coding)",
/* + PL:patches + */ 			"graph_hfh" => "Head for Heights (Graphics)",
/* + PL:patches + */ 			"directory" => "RPG-Directory",
/* + PL:patches + */ 			"stormgroupie" => "Stormgroupie",
/* + PL:patches + */ 			"code_templates" => "Templates 2 Go-Bereich",
/* + PL:patches + */ 			"writing_general" => "Writing-Bereich verwalten"
/* + PL:patches + */ 		);
/* + PL:patches + */ 
/* + PL:patches + */ foreach($aufgaben as $key => $value) {
/* + PL:patches + */ 	if($aufgabe[$key] == "1") {
/* + PL:patches + */ 		$query = $db->query("SELECT *, COUNT(*) as anzahl FROM mybb_threads WHERE fid = '10' AND thema = '$key' AND threadsolved = '0'");
/* + PL:patches + */ 		while($supthema = $db->fetch_array($query)) {
/* + PL:patches + */ 			if($supthema['anzahl'] != "0") {
/* + PL:patches + */ 				$support_alert .= "<div class=\"red_alert\"><a href=\"showthread.php?tid=$supthema[tid]\" target=\"_blank\">$supthema[username] hat eine Frage zum Bereich <span style=\"font-weight: bold;\">$value</span></a></div>";
/* + PL:patches + */ 			}
/* + PL:patches + */ 		}
/* + PL:patches + */ 	}
/* + PL:patches + */ }
/* + PL:patches + */ 
/* + PL:patches + */ 	if($aufgabe['code_templates'] == "1") {
/* + PL:patches + */ 		$query = $db->query("SELECT COUNT(*) AS anzahl FROM mybb_threads WHERE fid = '636' AND visible ='1'");
/* + PL:patches + */ 		$t2go = $db->fetch_field($query, "anzahl");
/* + PL:patches + */ 		if($t2go > "1") {
/* + PL:patches + */ 			$t2go_check = "<div class=\"red_alert\"><a href=\"http://storming-gates.de/forumdisplay.php?fid=636\" target=\"_blank\">Es gibt neu eingereichte Templates!</a></div>";
/* + PL:patches + */ 		}
/* + PL:patches + */ 	}
/* + PL:patches + */ 
/* + PL:patches + */ eval('$header_contests = "'.$templates->get('header_contests').'";');
/* + PL:patches + */ }
...
/* + PL:patches + */ // HEADER
/* + PL:patches + */ $path    = 'images/one/header';
/* + PL:patches + */ $files = array_diff(scandir($path), array('..', '.'));
/* + PL:patches + */ $filescount = count($files) + 1;
/* + PL:patches + */ $header_key = rand(2, $filescount);
/* + PL:patches + */ if(empty($mybb->user['fixedtheme'])) {
/* + PL:patches + */ $fixtheme = "<a href=\"misc.php?action=fixtheme&key={$header_key}\"><div class=\"arrow-up\">Design speichern</div></a>";
/* + PL:patches + */ } else {
/* + PL:patches + */ $header_key = $mybb->user['fixedtheme'];
/* + PL:patches + */ $fixtheme = "<a href=\"misc.php?action=fixtheme\"><div class=\"arrow-up\">Wechseldesign aktivieren</div></a>"; }
/* + PL:patches + */ $header_one = "images/one/header/{$files[$header_key]}";
/* + PL:patches + */ $header_colors = array("2" => "#972c13", "3" => "#424a94", "4" => "#53331a", "5" => "#5b7081", "6" => "#c06d3f", "7" => "#4c8380", "8" => "#647d84", "9" => "#d0cb6b", "10" => "#634851", "11" => "#3c908b", "12" => "#c88049", "13" => "#2d3c31", "14" => "#86c0c6");
/* + PL:patches + */ $header_color = $header_colors[$header_key];
/* + PL:patches + */ $header_colors_2 = array("2" => "#ce8551", "3" => "#b97678", "4" => "#311d1f", "5" => "#8f9193", "6" => "#ebba86", "7" => "#495c63", "8" => "#a79e98", "9" => "#7fa960", "10" => "#ad9989", "11" => "#be9586", "12" => "#6c4269", "13" => "#646943", "14" => "#b9c07e");
/* + PL:patches + */ $header_color_2 = $header_colors_2[$header_key];
/* + PL:patches + */ 
/* + PL:patches + */ // POSTBIT 
/* + PL:patches + */ if(empty($mybb->user['postbit']) || $mybb->user['postbit'] == "left") {
/* + PL:patches + */ $postdirec = "left";
/* + PL:patches + */ }
/* + PL:patches + */ if($mybb->user['postbit'] == "right") {
/* + PL:patches + */ $postdirec = "right";
/* + PL:patches + */ }
/* + PL:patches + */ 
/* + PL:patches + */ if($mybb->user['dimmed'] == "1") {
/* + PL:patches + */ $dimmed = "body { background: url('http://storming-gates.de/images/one/bgpattern_dimmed.png') }";
/* + PL:patches + */ $opac = ".9";
/* + PL:patches + */ } else {
/* + PL:patches + */ $dimmed = "body { background: url('images/one/bgpattern.png') }";
/* + PL:patches + */ $opac = "1";
/* + PL:patches + */ }
/* + PL:patches + */ 
/* + PL:patches + */      // GRAFIKGALERIEN
/* + PL:patches + */         // WICHTIG! f.fid = 'aaa' --> aaa anpassen mit ID des Steckbriefforums
/* + PL:patches + */ $header_one_ggallery = "<i class=\"icon-picture\"></i>";
/* + PL:patches + */         // Bei mehreren Steckbriefforen: f.fid = 'aaa' || f.fid = 'bbb'
/* + PL:patches + */         $grafiken = $db->query("
/* + PL:patches + */             SELECT *
/* + PL:patches + */             FROM ".TABLE_PREFIX."threads t
/* + PL:patches + */             LEFT JOIN ".TABLE_PREFIX."posts p ON (t.firstpost = p.pid)
/* + PL:patches + */             LEFT JOIN ".TABLE_PREFIX."forums f ON (f.fid = t.fid)
/* + PL:patches + */             WHERE (t.uid = '".$mybb->user['uid']."') AND (f.fid = '50' || f.fid = '46' || f.fid = '45' || f.fid = '44' || f.fid = '43' || f.fid = '42' || f.fid = '41' || f.fid = '47')
/* + PL:patches + */             LIMIT 1
/* + PL:patches + */         ");
/* + PL:patches + */         while($row = $db->fetch_array($grafiken)){
/* + PL:patches + */ 			 $grafikenurl = "/showthread.php?tid=$row[tid]&pid=$row[firstpost]#pid$row[firstpost]";
/* + PL:patches + */              $header_ggallery = "<div class=\"ggalerie\"> <a href=\"$grafikenurl\" target=\"_blank\" style=\"color: #ffffff;\">Grafik-Galerie</a></div>";
/* + PL:patches + */ $header_one_ggallery = "<a href=\"$grafikenurl\"><i class=\"icon-picture\"></i></a>";
/* + PL:patches + */         }    
/* + PL:patches + */ 
/* + PL:patches + */ 		// CODINGGALERIEN
/* + PL:patches + */ $header_one_cgallery = "<i class=\"icon-crop\"></i>";
/* + PL:patches + */         // WICHTIG! f.fid = 'aaa' --> aaa anpassen mit ID des Steckbriefforums
/* + PL:patches + */         // Bei mehreren Steckbriefforen: f.fid = 'aaa' || f.fid = 'bbb'
/* + PL:patches + */         $coding = $db->query("
/* + PL:patches + */             SELECT *
/* + PL:patches + */             FROM ".TABLE_PREFIX."threads t
/* + PL:patches + */             LEFT JOIN ".TABLE_PREFIX."posts p ON (t.firstpost = p.pid)
/* + PL:patches + */             LEFT JOIN ".TABLE_PREFIX."forums f ON (f.fid = t.fid)
/* + PL:patches + */             WHERE (t.uid = '".$mybb->user['uid']."') AND (f.fid = '80')
/* + PL:patches + */             LIMIT 1
/* + PL:patches + */         ");
/* + PL:patches + */         while($row = $db->fetch_array($coding)){
/* + PL:patches + */ 			 $codingurl = "/showthread.php?tid=$row[tid]&pid=$row[firstpost]#pid$row[firstpost]";
/* + PL:patches + */              $header_cgallery = "<div class=\"cgalerie\"> <a href=\"$codingurl\" target=\"_blank\" style=\"color: #ffffff;\">Coding-Galerie</a></div>";
/* + PL:patches + */ $header_one_cgallery = "<a href=\"$codingurl\"><i class=\"icon-crop\"></i></a>";
/* + PL:patches + */         }    
/* + PL:patches + */ 
/* + PL:patches + */        // WRITINGGALERIEN
/* + PL:patches + */         // WICHTIG! f.fid = 'aaa' --> aaa anpassen mit ID des Steckbriefforums
/* + PL:patches + */ $header_one_wgallery = "<i class=\"icon-book-open\"></i>";
/* + PL:patches + */         // Bei mehreren Steckbriefforen: f.fid = 'aaa' || f.fid = 'bbb'
/* + PL:patches + */         $write = $db->query("
/* + PL:patches + */             SELECT *
/* + PL:patches + */             FROM ".TABLE_PREFIX."threads t
/* + PL:patches + */             LEFT JOIN ".TABLE_PREFIX."posts p ON (t.firstpost = p.pid)
/* + PL:patches + */             LEFT JOIN ".TABLE_PREFIX."forums f ON (f.fid = t.fid)
/* + PL:patches + */             WHERE (t.uid = '".$mybb->user['uid']."') AND (f.fid = '226')
/* + PL:patches + */             LIMIT 1
/* + PL:patches + */         ");
/* + PL:patches + */         while($row = $db->fetch_array($write)){
/* + PL:patches + */ 			 $writeurl = "/showthread.php?tid=$row[tid]&pid=$row[firstpost]#pid$row[firstpost]";
/* + PL:patches + */              $header_wgallery = "<div class=\"wgalerie\"> <a href=\"$writeurl\" target=\"_blank\" style=\"color: #ffffff;\">Writing-Galerie</a></div>";
/* + PL:patches + */ $header_one_wgallery = "<a href=\"$writeurl\"><i class=\"icon-book-open\"></i></a>";
/* + PL:patches + */         }
...
/* + PL:patches + */ $query = $db->query("SELECT * FROM mybb_threads WHERE fid = '9' ORDER BY dateline DESC LIMIT 5");
/* + PL:patches + */ while($news = $db->fetch_array($query)) {
/* + PL:patches + */         $news['dateline'] = date("d.m.Y", $news['dateline']);
/* + PL:patches + */         $news['url'] = "<a href=\"showthread.php?tid=$news[tid]\" target=\"_blank\">$news[subject]</a>";
/* + PL:patches + */ 	$neuigkeit .= "<tr><td class=\"trow1\"><center><b>{$news['dateline']}</b></center> &raquo; {$news['url']}</td></tr>";
/* + PL:patches + */ }
// Set up some of the default templates
/* + PL:patches + */ // Username oder Spitzname?
/* + PL:patches + */ if(!empty($mybb->user['fid1'])) {
/* + PL:patches + */ $ansprache = $mybb->user['fid1'];
/* + PL:patches + */ }
/* + PL:patches + */ else {
/* + PL:patches + */ $ansprache = $mybb->user['username'];
/* + PL:patches + */ } 
/* + PL:patches + */ // Header je nach Usergruppe
/* + PL:patches + */ if($mybb->user['usergroup'] == 1) {
/* + PL:patches + */     eval('$header_revival = "'.$templates->get('header_revgast').'";');
/* + PL:patches + */     $headerava = "";
/* + PL:patches + */ }
/* + PL:patches + */ else {
/* + PL:patches + */     eval('$header_revival = "'.$templates->get('header_rev').'";');
/* + PL:patches + */     $headerava = "<img src=\"{$mybb->user['avatar']}\" />";
/* + PL:patches + */ }
eval('$headerinclude = "'.$templates->get('headerinclude').'";');
eval('$gobutton = "'.$templates->get('gobutton').'";');
eval('$htmldoctype = "'.$templates->get('htmldoctype', 1, 0).'";');
/* + PL:patches + */ $query = $db->query("SELECT * FROM mybb_threads
/* + PL:patches + */ LEFT JOIN mybb_forums on mybb_threads.fid = mybb_forums.fid
/* + PL:patches + */ WHERE mybb_forums.parentlist LIKE '%,49,%'
/* + PL:patches + */ ORDER BY mybb_threads.dateline DESC
/* + PL:patches + */ LIMIT 5");
/* + PL:patches + */ while($partner = $db->fetch_array($query)) {
/* + PL:patches + */ 	$partnerforum .= "<tr><td class=\"trow1\"><a href=\"showthread.php?tid={$partner['tid']}\" target=\"_blank\">{$partner['subject']}</a></td></tr>";
/* + PL:patches + */ }
/* + PL:patches + */ eval("\$sisterbuttons = \"".$templates->get("sisterbuttons")."\";");
